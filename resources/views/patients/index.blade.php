        ...
        <form method="GET" action="{{ route('patients.index') }}" class="flex gap-3">
            <div class="relative">
                ...
                <input type="text" name="search" value="{{ $search ?? '' }}"
                       ...
                       class="pl-10 pr-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm placeholder-slate-400 focus:outline-none focus:border-cyan-500 w-72 transition-colors"/>
            </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-slate-800">
                    ...
                    <th class="text-left text-slate-400 font-medium px-6 py-3.5 hidden sm:table-cell">Téléphone</th>
                    <th class="text-left text-slate-400 font-medium px-6 py-3.5 hidden md:table-cell">Date naissance</th>
                    <th class="text-left text-slate-400 font-medium px-6 py-3.5 hidden lg:table-cell">Sexe</th>
                    ...
                    <tr class="hover:bg-slate-800/50 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="text-white font-medium">{{ $patient->prenom }} {{ $patient->nom }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-slate-300 hidden sm:table-cell">{{ $patient->telephone }}</td>
                        <td class="px-6 py-4 text-slate-400 hidden md:table-cell">{{ \Carbon\Carbon::parse($patient->date_naissance)->format('d/m/Y') }}</td>
                        <td class="px-6 py-4 hidden lg:table-cell">
                            <span class="px-2 py-1 rounded-md text-xs font-medium border
                                {{ $patient->sexe === 'M' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-pink-500/10 text-pink-400 border-pink-500/20' }}">
