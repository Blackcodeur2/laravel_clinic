        ...
        <div class="rounded-2xl bg-slate-900 border border-slate-800 p-6">
            <form method="POST" action="{{ route('services.update', $service) }}" class="space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-slate-300 text-sm font-medium mb-1.5">Code <span class="text-red-400">*</span></label>
                        <input type="text" name="code" value="{{ old('code', $service->code) }}" required
                               class="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm font-mono focus:outline-none focus:border-cyan-500 transition-colors @error('code') border-red-500 @enderror"/>
                        @error('code')<p class="mt-1 text-red-400 text-xs">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-slate-300 text-sm font-medium mb-1.5">Prix (FCFA) <span class="text-red-400">*</span></label>
                ...
                <div>
                    <label class="block text-slate-300 text-sm font-medium mb-1.5">Nom du service <span class="text-red-400">*</span></label>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                              class="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-cyan-500 transition-colors resize-none">{{ old('description', $service->description) }}</textarea>
                </div>
